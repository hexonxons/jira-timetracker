...
- **Токен не покидает сервер.** В браузер уходит только признак «токен задан». Менять настройки из браузера
  нельзя: в режиме сервиса `PUT /api/settings` и `PUT /api/teams` возвращают `403`. Иначе любой посетитель мог
  бы подменить URL Jira и увести токен.
...
Режим сервиса включается, когда задан `JTT_JIRA_URL`. Все настройки задаются переменными окружения.
...
- **Проверка здоровья:** `GET /healthz` → `{"status": "ok", "managed": true, "configErrors": []}`.
